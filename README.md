...
## 9. The wishing sky
Six stars scattered over a night sky — `wishingSky.wishes` in `config.js` is a list of `title`/`text` pairs, one per star, scattered to a new random spot each time the page loads. Tapping the very first star starts a song (`wishingSky.song`/`songTitle`) that keeps playing uninterrupted as she taps the rest — it won't restart on later taps. Once all six have been found and she closes that message, a moon quietly appears in the top-right corner — tapping it reveals one final note (`wishingSky.moon.text`).
